Shader "PIDI Shaders Collection/Advanced Skin Shader 2/Realistic/Simple (UMA Compatible)" {
	Properties {
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputSkin o) {
			...
			o.Specular = fixed3(0.24,0.24,0.24)*microSkin.b;


			
			o.Smoothness = lerp(spec.a * _Glossiness * microSkin.b, saturate(microSkin.b * _PSkinMicroSkinSmoothness + spec.a * _Glossiness * microSkin.b), 1 - pow(abs(dot(IN.vNormal, IN.viewDirW)), _PSkinDualLobeTransition));
